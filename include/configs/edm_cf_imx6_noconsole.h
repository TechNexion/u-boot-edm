...
#define CONFIG_SYS_PROMPT	       "EDM_CF_IMX6_NOCONSOLE=> "
...
#define CONFIG_EXTRA_ENV_SETTINGS					\
		...
		"bootargs_base=setenv bootargs console=\0"	\
		...
		"android_hdmi_bootargs=edm-tn "		\
			...
			"vmalloc=400M androidboot.console= "		\
			"androidboot.hardware=fairy\0"			\
		"android_lvds_bootargs=edm-tn "		\
			"root=/dev/mmcblk0p1 rootwait rw init=/init "		\
			"video=mxcfb0:dev=ldb,LDB-WSVGA,if=RGB24 " \
			"video=mxcfb1:off video=mxcfb2:off fbmem=28M "		\
			"vmalloc=400M androidboot.console= "		\
			"androidboot.hardware=fairy\0"			\
		"android_lcd_bootargs=edm-tn "		\
			"root=/dev/mmcblk0p1 rootwait rw init=/init "		\
...
#define CONFIG_EXTRA_ENV_SETTINGS \
	"mtdparts=" MTDPARTS_DEFAULT "\0" \
	"setbootdev=if test ${bootmode} = NAND; then run nandboot; " \
		"else if test ${bootmode} = MMC; then run mmcboot;else " \
		"if test ${bootmode} = SATA; then run sataboot;fi;fi;fi\0" \
	"mmcboot=setenv devtype mmc;setenv devno 0; setenv rootdevice " \
		"\'root=/dev/mmcblk0p2 rootwait rootfstype=ext4\'; " \
		"mmc dev 0; mmc rescan\0" \
	"sataboot=setenv devtype sata;setenv devno 0; setenv rootdevice " \
		"\'root=/dev/sda2 rootwait rw rootfstype=ext4\';\0" \
	"nandboot=setenv devtype nand;setenv devno 0; setenv loadbootenv; " \
		"setenv loadramdisk; setenv rootdevice \'ubi.mtd=5 root=ubi0:rootfs " \
		"rootfstype=ubifs ${mtdparts}\';setenv loaduimage " \
		"\'nand read ${loadaddr} 0x320000 0x400000;\'\0" \
	"bootargs_base=\0" \
	...
	"loadbootenv=if fatload ${devtype} ${devno} ${loadaddr} ${bootenv}; " \
		"then; else ext2load ${devtype} ${devno} ${loadaddr} ${bootenv}; fi\0" \
	"loadramdisk=if fatload ${devtype} ${devno} ${initrdaddr} boot/uramdisk.img; " \
		"then; else ext2load ${devtype} ${devno} ${initrdaddr} boot/uramdisk.img; fi\0" \
	"loaduimage=if fatload ${devtype} ${devno} ${loadaddr} boot/uImage; " \
		"then; else ext2load ${devtype} ${devno} ${loadaddr} boot/uImage; fi\0" \
	"loaduimage_raw=${devtype} read ${loadaddr} 0x800 0x4000\0 "	\
	"importbootenv=echo Importing environment...; " \
		"env import -t $loadaddr $filesize\0" \
	"splashimage=0x10800000\0"				\
	"splashimage_init_block=0x410\0"			\
	"splashimage_blkcnt=0x3F0\0"			\
	"splashimage_file_name=boot/out.bmp.gz\0"		\
        "splashpos=m,m\0"
